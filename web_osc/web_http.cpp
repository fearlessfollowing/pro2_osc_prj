...
MG_INTERNAL 
void mg_call(struct mg_connection *nc, mg_event_handler_t ev_handler, int ev, void *ev_data) 
{
    if (ev_handler == NULL) {
        /*
...
/* LCOV_EXCL_STOP */
    ...
    if (ev != MG_EV_POLL) {
        DBG(("%p after %s flags=%lu rmbl=%d smbl=%d", nc,ev_handler == nc->handler ? "user" : "proto", nc->flags,
            (int) nc->recv_mbuf.len, (int) nc->send_mbuf.len));
    }
}
...
void mg_close_conn(struct mg_connection *conn) 
{
    ...
    conn->iface->vtable->destroy_conn(conn);    /* 关闭连接的套接字 */
...
void mg_mgr_init_opt(struct mg_mgr *m, void *user_data, struct mg_mgr_init_opts opts) 
{
    ...
    printf("--> MG_NET_IF %d mg_num_ifaces %d\n", MG_NET_IF, mg_num_ifaces);

    /* Ignore SIGPIPE signal, so if client cancels the request, it won't kill the whole process. */
...

    {
        ...
        if (opts.num_ifaces == 0) {
            opts.num_ifaces = mg_num_ifaces;        /* 初始化接口数 */
            opts.ifaces = mg_ifaces;                /* 接口对象指针数组 */
        }
...
/*************************************************************************
** 方法名称: mg_create_connection_base
** 方法功能: 创建并初始化连接对象
** 入口参数: 
**      mgr   - 管理器对象指针
**      callback - 连接对象的事件处理
**      opts  - 用于该连接的套接字选项
** 返回值:   连接对象指针(struct mg_connection*)
** 调 用:   局部调用
*************************************************************************/
MG_INTERNAL 
struct mg_connection *mg_create_connection_base(struct mg_mgr *mgr, 
                                                mg_event_handler_t callback,
                                                struct mg_add_sock_opts opts) 
{
...
*************************************************************************/
MG_INTERNAL 
struct mg_connection *mg_create_connection(struct mg_mgr *mgr, 
                                            mg_event_handler_t callback,
                                            struct mg_add_sock_opts opts) 
{
 ...
 */
MG_INTERNAL 
int mg_parse_address(const char *str, union socket_address *sa, int *proto, char *host, size_t host_len) 
{
...
MG_INTERNAL struct mg_connection *mg_do_connect(struct mg_connection *nc, int proto, union socket_address *sa) 
{
    ...
    DBG(("%p %s://%s:%hu", nc, proto == SOCK_DGRAM ? "udp" : "tcp", inet_ntoa(sa->sin.sin_addr), ntohs(sa->sin.sin_port)));
...
#if MG_ENABLE_TUN
    if (mg_strncmp(mg_mk_str(address), mg_mk_str("ws://"), 5) == 0 || mg_strncmp(mg_mk_str(address), mg_mk_str("wss://"), 6) == 0) {
...
#if MG_ENABLE_TUN
struct mg_iface_vtable *mg_ifaces[] = {&mg_default_iface_vtable, &mg_tun_iface_vtable};
#else
...
static 
sock_t mg_open_listening_socket(union socket_address *sa, int type, int proto) 
{
...
static void mg_write_to_socket(struct mg_connection *nc) 
{
    ...
    if (nc->flags & MG_F_UDP) {
        int n = sendto(nc->sock, io->buf, io->len, 0, &nc->sa.sa, sizeof(nc->sa.sin));
...
static void mg_handle_tcp_read(struct mg_connection *conn) 
{
    ...
    {
        n = (int) MG_RECV_FUNC(conn->sock, buf, recv_avail_size(conn, MG_TCP_RECV_BUFFER_SIZE), 0);
...
void mg_mgr_handle_conn(struct mg_connection *nc, int fd_flags, double now) 
{
    ...
    if (fd_flags & _MG_F_FD_CAN_READ) {
        if (nc->flags & MG_F_UDP) {     /* UDP连接 */
            mg_handle_udp_read(nc);
        } else {
            if (nc->flags & MG_F_LISTENING) {   /* 如果该连接处于监听状态，则建立连接 */
                mg_accept_conn(nc);
            } else {    /* 从连接端口读取数据 */
...
time_t mg_socket_if_poll(struct mg_iface *iface, int timeout_ms) 
{
    ...
    for (nc = mgr->active_connections, num_fds = 0; nc != NULL; nc = tmp) {
        ...
        if (nc->sock != INVALID_SOCKET) {
            ...
            if (!(nc->flags & MG_F_WANT_WRITE) &&
                nc->recv_mbuf.len < nc->recv_mbuf_limit && (!(nc->flags & MG_F_UDP) || nc->listener == NULL)) {
    ...
    */
        ...
        if (nc->sock != INVALID_SOCKET) {
            if (num_ev > 0) {
                fd_flags = (FD_ISSET(nc->sock, &read_set) &&
                            ...
                            ? _MG_F_FD_CAN_READ
                            : 0) |
                    (FD_ISSET(nc->sock, &write_set) ? _MG_F_FD_CAN_WRITE : 0) | (FD_ISSET(nc->sock, &err_set) ? _MG_F_FD_ERROR : 0);
            }
        }
        tmp = nc->next;
    ...
    /* 
     * 关闭无用的连接
     * 1.连接标志设置为MG_F_CLOSE_IMMEDIATELY
     * 2.连接标记设置为MG_F_SEND_AND_CLOSE但无数据需要发送的
     */
    for (nc = mgr->active_connections; nc != NULL; nc = tmp) {
        tmp = nc->next;
...
int mg_parse_http(const char *s, int n, struct http_message *hm, int is_req) 
{
    ...
    */
    ...
    if (hm->body.len == (size_t) ~0 && is_req &&
        mg_vcasecmp(&hm->method, "PUT") != 0 && mg_vcasecmp(&hm->method, "POST") != 0) {
        hm->body.len = 0;
        hm->message.len = len;
    }
...
static void mg_http_transfer_file_data(struct mg_connection *nc) 
{
    ...
    if (pd->file.type == DATA_FILE) {
        ...
        if (to_read == 0) {
            printf("io->len %ld left %ld to_read %ld pd->file.path %s\n",io->len,left,to_read,pd->file.path);
            ...
            printf("a mg_http_transfer_file_data to_read %ld"
                       " pd->file.sent %ld pd->file.cl %ld n %ld\n",
               to_read, pd->file.sent,pd->file.cl ,n);
            mg_send(nc, buf, n);
            pd->file.sent += n;
        } else {
            printf("b mg_http_transfer_file_data to_read %ld"
                       " pd->file.sent %ld pd->file.cl %ld n %ld\n",
               to_read, pd->file.sent,pd->file.cl ,n);
...
static void mg_http_call_endpoint_handler(struct mg_connection *nc, int ev, struct http_message *hm) 
{
    ...
    if (pd->endpoint_handler == NULL || ev == MG_EV_HTTP_REQUEST) {
        pd->endpoint_handler = ev == MG_EV_HTTP_REQUEST ? mg_http_get_endpoint_handler(nc->listener, &hm->uri) : NULL;
    }
    
    mg_call(nc, pd->endpoint_handler ? pd->endpoint_handler : nc->handler, ev, hm);
}
...
static void mg_http_handler2(struct mg_connection *nc, int ev, void *ev_data, struct http_message *hm) 
{
#else  /* !__XTENSA__ */
void mg_http_handler(struct mg_connection *nc, int ev, void *ev_data) 
{
...
/*************************************************************************
** 方法名称: mg_set_protocol_http_websocket
** 方法功能: 为指定的连接设置http协议处理器
** 入口参数: 
**      nc   - 连接对象指针
** 返回值:   无
** 调 用:   全局调用
                        ...
                        const struct mg_str extra_headers)
{
    ...
    if (mg_stat(path, &st) != 0 || (pd->file.fp = mg_fopen(path, "rb")) == NULL) {
        ...
        range[0] = '\0';
        if (range_hdr != NULL &&
            (n = mg_http_parse_range_header(range_hdr, &r1, &r2)) > 0 && r1 >= 0 && r2 >= 0) {
    ...
    _XOPEN_SOURCE >= 600
            ...
            printf("mg_http_serve_file e st.st_size %ld cl %ld r2 %ld r1 %ld\n",
               st.st_size,cl,r2, r1);
        } else {
            printf("no range\n");
        }
    ...
    pd->file.type = DATA_FILE;
      ...
      printf("mg_http_serve_file cl %ld"
                     " f path %s\n",cl,path);

    snprintf(pd->file.path,sizeof(pd->file.path),"%s",path);

      mg_http_transfer_file_data(nc);
      printf("mg_http_serve_file g\n");
  }
}
...
int mg_http_parse_header(struct mg_str *hdr, const char *var_name, char *buf, size_t buf_size) 
{
    ...
    for (s = hdr->p; s != NULL && s + n < end; s++) {
        if ((s == hdr->p || s[-1] == ch || s[-1] == ch1) && s[n] == '=' && !strncmp(s, var_name, n))
            break;
    }
...
int mg_http_check_digest_auth(struct http_message *hm, const char *auth_domain, FILE *fp) 
{
    ...
    if (hm == NULL || fp == NULL || (hdr = mg_get_http_header(hm, "Authorization")) == NULL ||
    ...
    */
    while (fgets(buf, sizeof(buf), fp) != NULL) {
        if (sscanf(buf, "%[^:]:%[^:]:%s", f_user, f_domain, f_ha1) == 3 && strcmp(user, f_user) == 0 &&
...
static size_t mg_url_encode(const char *src, size_t s_len, char *dst, size_t dst_len) 
{
    ...
    for (i = j = 0; dst_len > 0 && i < s_len && j + 2 < dst_len - 1; i++, j++) {
        if (isalnum(*(const unsigned char *) (src + i)) || strchr(dont_escape, *(const unsigned char *) (src + i)) != NULL) {
            dst[j] = src[i];
        } else if (j + 3 < dst_len) {
            dst[j] = '%';
...
MG_INTERNAL void mg_send_http_file(struct mg_connection *nc, char *path,
    ...
    DBG(("%p %.*s [%s] exists=%d is_dir=%d is_dav=%d is_cgi=%d path_info->len %d index=%s", nc,
       (int) hm->method.len, hm->method.p, path, exists, is_directory, is_dav,
       is_cgi, path_info->len,index_file ? index_file : ""));
                   ...
                   struct mg_serve_http_opts opts) 
{
    ...
    if (mg_vcmp(&hm->proto, "HTTP/1.1") != 0 || ((hdr = mg_get_http_header(hm, "Connection")) != NULL &&
...
struct mg_connection *mg_connect_http_base(struct mg_mgr *mgr, mg_event_handler_t ev_handler,
                                            struct mg_connect_opts opts, const char *schema, 
                                            const char *schema_ssl, const char *url, 
                                            const char **path, char **user, char **pass, char **addr) 
{
...
MG_INTERNAL void mg_ws_handler(struct mg_connection *nc, int ev, void *ev_data) 
{
    ...
    switch (ev) {
        ...
        case MG_EV_POLL: { /* Ping idle websocket connections */
