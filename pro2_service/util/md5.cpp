...
bool check_file_key_md5(const char *file_path)
{
    ...
    char md5_check[MD5_STR_LEN + 1];
    MD5_CTX md5;

    unsigned int file_size = get_file_size(file_path );
    ...
    while (left_size > 0) {
        read_size = (left_size < READ_DATA_SIZE) ? left_size : READ_DATA_SIZE;
        ret = read(fd, data, read_size);
        if (ret > 0) {
            MD5Update(&md5, data, ret);
            left_size -= ret;
        } else {
            fprintf(stderr, "err read size %d\n",ret);
            goto EXIT;
        }
    }

    memset(md5_check,0,sizeof(md5_check));
    ret = read(fd, md5_check, MD5_STR_LEN);
...
#ifdef ENABLE_TEST_KEY
    MD5Update(&md5,(unsigned char *)TEST_KEY, strlen((char *)TEST_KEY));
#endif
    MD5Final(&md5, md5_value);
    ...
    if(strcmp(md5_check, md5_str) == 0) {
		...
		fprintf(stderr, "mismatch read md5 check(%s %s)", md5_check, md5_str);
	}
...
int write_file_key_md5(const char *file_path)
{
    ...
    // convert md5 value to md5 string
    for (i = 0; i < MD5_SIZE; i++) {
		printf("0x%x ", md5_value[i]);
        snprintf(md5_str + i*2, 2+1, "%02x", md5_value[i]);
    }

    printf("write md5_str is %s strlen %zd\n", md5_str, strlen(md5_str));

	write_len = write(fd, md5_str, strlen(md5_str));
	if (write_len != strlen(md5_str)) {
		printf("write tail mismtach(%d %zd)\n", write_len, strlen(md5_str));
		ret = -1;
	} else {
		ret = 0;
	}
