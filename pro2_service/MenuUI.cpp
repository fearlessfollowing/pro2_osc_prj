            ...
            if (pTmpPicVidCfg) {
                ...
                if (pAebSetItem && pAebPicVidCfg) {
                    ...
                    LOGDBG(TAG, "Current AEB info: hdr_count: %d, min_ev: %d, max_ev: %d", 
                                 pTmpAeb->hdr_count, pTmpAeb->min_ev, pTmpAeb->max_ev);
...
void MenuUI::dispPicVidCfg(PicVideoCfg* pCfg, bool bLight)
{
    ...
    LOGDBG(TAG, "dispPicVidCfg Current item[%s], val[%d]", pCfg->pItemName, pCfg->iCurVal);
        ...
        case MENU_SHOW_SPACE: { /* 如果选中的SD卡或者USB硬盘进入格式化指示菜单 */
            ...
            if (!strncmp(gStorageInfoItems[iIndex]->pStVolumeInfo->cVolName, "mSD", strlen("mSD"))) {
                ...
                LOGDBG(TAG, "You selected [%s] Card!!", 
                         gStorageInfoItems[iIndex]->pStVolumeInfo->cVolName);                
...
bool MenuUI::check_cur_menu_support_key(int iAppKey)
{
	for (int i = 0; i < SYS_MAX_BTN_NUM; i++) {
		if (mMenuInfos[cur_menu].mSupportkeys[i] == iAppKey)
			return true;
	}
	return false;
}
