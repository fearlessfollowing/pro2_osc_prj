...
bool MenuUI::start_speed_test()
{
    ...
    //     setCurMenu(MENU_SPEED_TEST); mActionInfo
    //     ret = true; ACTON_INFO
    // }
    return ret;
}
...
bool MenuUI::start_live_rec(const struct _action_info_ * mAct, ACTION_INFO *dest)
{
    ...
    if (checkStorageSatisfy(ACTION_LIVE)) {
        ...
        Log.d(TAG, "[%s: %d] >>>>>>>>>> no Storage", __FILE__, __LINE__);
    }
    return bAllow;
}
            ...
            if (pTmpPicVidCfg) {
                ...
                if (pAebSetItem && pAebPicVidCfg) {
                    ...
                    if (strcmp(pTmpPicVidCfg->pItemName, TAKE_PIC_MODE_CUSTOMER)) {
                        if (false == ((*pTakePicJson)["parameters"]["hdr"].isNull()) ) {
                ...
                if (false == (*pTakePicJson)["parameters"]["properties"].isNull()) {
                    ...
                    if (checkVidLiveStorageSpeed() == false) {
                        ...
                        if (pTmpPicVidCfg) {
                            ...
                            if (!strcmp(pTmpPicVidCfg->pItemName, TAKE_VID_MOD_CUSTOMER)) {
                                ...
                                sendRpc(ACTION_CUSTOM_PARAM, 0, &mProCfg->get_def_info(KEY_ALL_VIDEO_DEF)->stProp);
                            ...
                            if (pTakeVidJson) {
                                if (false == (*pTakeVidJson)["properties"].isNull()) {
                ...
                if (iSaveMode == LIVE_SAVE_NONE) {
                    ...
                    if (checkStorageSatisfy(option)) {  /* 对于不需要存片的挡位直接返回true */
                        if (checkVidLiveStorageSpeed() == false) {
                            ...
                            if (false == (*pTakeLiveJson)["parameters"]["properties"].isNull()) {
                                Log.d(TAG, "[%s: %d] -----------> Send Takepic Customer args First", __FILE__, __LINE__);
                                sendRpc(ACTION_CUSTOM_PARAM, 0, pTakeLiveJson);
                            }

                            // if (true == check_live_save(pTakeLiveJson)) {
...
void MenuUI::writeJson2File(const char* filePath, Json::Value& jsonRoot)
{
    ...
    if (reader.parse(jsonstr.c_str(), *(pRoot.get()), false)) {
        ...
        if (pTmpCfg) {
            Log.d(TAG, "[%s: %d] Update  Cutomer Json Command", __FILE__, __LINE__);
        ...
        case CONTROL_SET_CUSTOM: {    /* 设置Customer模式的值 */
            switch (type) {
                ...
                case ACTION_VIDEO: {     /* 设置录像模式下的Customer */
                    ...
                    writeJson2File(TAKE_VID_TEMPLET_PATH, vidRoot);
                    break;
                }
                ...
                case ACTION_LIVE: {      /* 直播模式下的Customer */
                    ...
                    writeJson2File(TAKE_LIVE_TEMPLET_PATH, liveRoot);
                    break;
                }
...
void MenuUI::getShowStorageInfo()
{
    ...
    vector<Volume*>& localVols = vm->getCurSavepathList();
    vector<Volume*>& remoteVols = vm->getRemoteVols();
   ...
   /* 将本地及远端的存储设备信息加入到mShowStorageList列表 */
    for (u32 i = 0; i < localVols.size(); i++) {
        ...
        if (access(localVols.at(i)->pMountPath, F_OK) != -1) {

            statfs(localVols.at(i)->pMountPath, &diskInfo);
            ...
            sprintf(tmpVolume->cVolName, "%s", (localVols.at(i)->iVolSubsys == VOLUME_SUBSYS_SD) ? "sd": "usb");
            tmpVolume->uTotal = totalsize >> 20;                 /* 统一将单位转换MB */
            tmpVolume->uAvail = tmpVolume->uTotal - used_size;
            tmpVolume->iType = VOLUME_TYPE_NV;
            
            tmpVolume->pMountPath = localVols.at(i)->pMountPath;                /* 挂载路径 */
            strcpy(tmpVolume->cDevNode, localVols.at(i)->cDevNode);             /* 设备节点名称 */


            mShowStorageList.push_back(tmpVolume);
        
        } else {
            Log.d(TAG, "%s not access", localVols.at(i)->pMountPath);
        }
    }
    ...
    {
        {
            ...
            for (u32 i = 0; i < remoteVols.size(); i++) {
                ...
                if (remoteVols.at(i)->uTotal > 0) {
                    ...
                    /* 名称不能超过3位 */
                    snprintf(tmpVolume->cVolName, sizeof(tmpVolume->cVolName), "%s", remoteVols.at(i)->cVolName);
                    tmpVolume->uTotal = remoteVols.at(i)->uTotal;
                    tmpVolume->uAvail = remoteVols.at(i)->uAvail;
                    tmpVolume->iIndex = remoteVols.at(i)->iIndex;
...
void MenuUI::calcRemainSpace(bool bUseCached)
{
 ...
 
    {
        ...
        if (vm->checkLocalVolumeExist()) {
            ...
            convSize2LeftNumTime(vm->getLocalVolLeftSize(bUseCached), CALC_MODE_TAKE_PIC);
        } else {
...
int MenuUI::getOneGroupPicSize(Json::Value& picRoot)
{
    /* 
     * 1.只存原片
     * 2.原片加拼接(Raw, jpeg, raw+jpeg),一共有6种情况
     */
...
void MenuUI::disp_org_rts(Json::Value& jsonCmd, int hdmi)
{
    ...
    if (jsonCmd["stiching"].isNull() == false) {
        rts = 1;
    }
	...
	switch (cur_menu) {
		...
		default: {
			...
			if (check_state_in(STATE_LOW_BAT)) {				
				Log.d(TAG, "STATE_LOW_BAT not allow (0x%x %d)", mCamState, disp_type->type);
